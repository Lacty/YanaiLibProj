...
void Font::drawCenter(const std::string& str, const float x, const float y, const int size) {
  ...
  float offset_y = (float)font_.BBox(str.c_str(), (int)str.size()).Upper().Y() * 0.5f;
  font_.Render(str.c_str(), (int)str.size(), FTPoint(x - offset_x, y - offset_y));
}
...
void Font::drawCenter(const wchar_t* str, const float x, const float y, const int size) {
  ...
  float offset_y = (float)font_.BBox(str, -1).Upper().Y() * 0.5f;
  font_.Render(str, -1, FTPoint(x - offset_x, y - offset_y));
}